...
先看效果再决定要不要用。演示里的 Word 是**真的 docx**（内容假、容器真），所以「和上一版比」点得动。
...
读不出来的时候也直说，不编：pdf / xlsx / pptx 现在解不出正文（那一轮连【和上一版比】都是灰的），
...
- 「和上一版比」只解 **docx** 与纯文本（md/tex/qmd/txt/csv/html）。pdf / xlsx / pptx 读不出正文，
  那一轮的按钮是灰的；超长文档只比前面 4000 段（面板上会说明被截断了）。
...
- 本仓库的代码由 **DeepSeek Harness 中的 AI Agent 生成**，由仓库所有者审阅、验证并发布；
  每次提交的验证方式记录在 `CHANGELOG.md`（不需要浏览器的两套自检，共 369 项）。
