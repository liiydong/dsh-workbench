 ...
 *         读一版产物的**正文**，给「这一版比上一版改了什么」做地基：
 *           docx  → zip → word/document.xml → 段落 / 表格行 / 标题
 *           xlsx  → zip → workbook + rels + sharedStrings + 各表 xml → 一行一块（格子用竖线连）
 *           md/tex/qmd/txt/csv/html → 当纯文本读
 *         读不出来的类型（pdf/pptx/老的 .xls）如实说读不出，不编内容。
 ...
 */
const READABLE_EXT = new Set(['.docx', '.xlsx', '.xlsm', '.md', '.markdown', '.tex', '.qmd', '.txt', '.csv', '.html', '.htm'])
/** OOXML 电子表格（xlsx / xlsm 都是 zip，能解）。老的 .xls 是另一种二进制容器，解不了。 */
...
/**
 * `A1` 这种单元格引用 → 列号（0 起）。取不到字母就当第 0 列。
 *
 * @param ref - 单元格引用。
 * @returns 列号。
 */
...
/**
 * 一个 xlsx（OOXML 电子表格）→ 块数组。
 *
 * 表格和文档不一样：它的单位是**行**，所以一块就是一行，单元格之间用竖线连起来 ——
 * 于是「改了哪个格子」在词级高亮里会自己跳出来，和 Word 的段落 diff 用的是同一套东西。
 *
 * 每个工作表先落一个 `sheet` 标记块，再是这个表的行。解析器只认四样东西：
 * `xl/workbook.xml`（有哪些表）、`xl/_rels/workbook.xml.rels`（表对应哪个 xml）、
 * `xl/sharedStrings.xml`（共享字符串表）、表自己的 xml（行与单元格）。
 * 样式、格式、图表一律不看——那是「长什么样」，不是「写了什么」。
 *
 * 单元格取值：共享字符串、内联字符串、公式算出的字符串、布尔、错误码、数字原样。
 * 公式本身不取（`<f>`），没算完的缓存也就没有值——如实留空，不猜。
 * 日期也如实留成数字：判它是不是日期要看 styles.xml 的数字格式，那一步容易猜错。
 *
 * @param buffer - xlsx 文件的字节。
 * @returns 块数组；解析不出结构时返回 null（由调用方给一句人话）。
 */
function xlsxBlocks(buffer) {
	const workbook = unzipEntry(buffer, 'xl/workbook.xml')
	if (workbook === null) return null
	const rels = unzipEntry(buffer, 'xl/_rels/workbook.xml.rels')
	const shared = unzipEntry(buffer, 'xl/sharedStrings.xml')

	/** 共享字符串表：每个 `<si>` 里的所有 `<t>` 拼成一条。 */
	const strings = []
	if (shared !== null) {
		for (const si of shared.toString('utf8').match(/<si\b[^>]*>[\s\S]*?<\/si>|<si\b[^>]*\/>/g) ?? []) {
			const parts = si.match(/<t\b[^>]*>([\s\S]*?)<\/t>/g) ?? []
			strings.push(parts.map((part) => decodeEntities(part.replace(/^<t\b[^>]*>/, '').replace(/<\/t>$/, ''))).join(''))
		}
	}
	/** rId → 表 xml 在包里的路径。Target 可能是相对的，也可能写成 `/xl/…`。 */
	...
	if (rels !== null) {
		for (const m of rels.toString('utf8').matchAll(/<Relationship\b[^>]*>/g)) {
			...
			const raw = decodeEntities(target[1]).replace(/\\/g, '/').replace(/^\/+/, '')
			targets.set(id[1], raw.startsWith('xl/') ? raw : `xl/${raw}`)
		}
	}
	...
	const cellText = (cell) => {
		...
		if (type === 'inlineStr') {
			...
			return decodeEntities((inline.match(/<t\b[^>]*>([\s\S]*?)<\/t>/g) ?? []).map((t) => t.replace(/^<t\b[^>]*>/, '').replace(/<\/t>$/, '')).join(''))
		}
		if (type === 's') return raw === undefined ? '' : strings[Number(raw)] ?? ''
	...
	const blocks = []
	for (const sheet of workbook.toString('utf8').match(/<sheet\b[^>]*\/?>/g) ?? []) {
		const name = /\bname="([^"]*)"/.exec(sheet)
		const rid = /\br:id="([^"]*)"/.exec(sheet)?.[1] ?? ''
		const path = targets.get(rid) ?? `xl/worksheets/sheet${blocks.length + 1}.xml`
		const xml = unzipEntry(buffer, path)
		const label = decodeEntities(name === null ? `工作表 ${blocks.length + 1}` : name[1])
		blocks.push({ kind: 'sheet', text: label })
		if (xml === null) continue
		for (const row of xml.toString('utf8').match(/<row\b[^>]*>[\s\S]*?<\/row>/g) ?? []) {
			const cells = []
			for (const cell of row.match(/<c\b[^>]*>[\s\S]*?<\/c>|<c\b[^>]*\/>/g) ?? []) {
				const ref = /\br="([A-Z]+)\d+"/.exec(cell)
				const at = ref === null ? cells.length : columnOf(`${ref[1]}1`)
				while (cells.length < at) cells.push('')
				cells[at] = cellText(cell).replace(/\s+$/, '')
			}
			// 整行都空的行不进对比：它在两份里都不占位置，比出来只会是噪声。
			if (cells.length === 0 || cells.every((cell) => cell === '')) continue
			blocks.push({ kind: 'row', text: cells.join(' | ') })
		}
	}
	return blocks
}
...
async function routeText(url, res) {
	...
	if (!READABLE_EXT.has(ext)) {
		return send(res, 400, { error: `读不出 ${ext === '' ? '这种' : ext} 类型的内容（支持 docx / xlsx 与 md/txt 一类纯文本）` })
	}
	let info
	try {
...
const kind = ext.replace('.', '')
	if (SPREADSHEET_EXT.has(ext)) {
		const blocks = xlsxBlocks(buffer)
		if (blocks === null) {
			return send(res, 200, {
				ok: false,
				error: '这个表格里找不到 xl/workbook.xml——可能不是 xlsx 文件，或者它被压缩/加密过',
				file,
				path: target,
				kind,
				blocks: []
