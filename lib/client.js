...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 段类型的左侧标记（标题加粗、表格行等宽）。 */
		const BLOCK_MARK = { h1: 'H1', h2: 'H2', h3: 'H3', h4: 'H4', li: '•', row: '▤', table: '▦', p: '' }
		...
		function IterationTab({ palette, view, patch }) {
			 ...
			 */
			const DIFFABLE_EXT = new Set(['.docx', '.md', '.markdown', '.tex', '.qmd', '.txt', '.csv', '.html', '.htm'])
			...
			const cannotWhy =
				picked === null || defaultBase === null
					...
					: DIFFABLE_EXT.has(extOf(diffTarget))
						? ''
						: `这种类型读不出正文（只支持 docx 与 md/txt 一类纯文本）`
